...
import { decrement, increment, Changecolor, onLike, addLike,adddLike } from './Plus.js'
...
const Intoe = () => {
    ...
    const date = useSelector(store => store.todo.date)
    console.log(data);
   ...
   <div className='border-black border-b-2 py-3'></div>
    ...
    <div>
        ...
        <div>
            ...
            {data.map((v) => (
        <div key={v.id}>
        <button   onClick={() => { dispatch(increment()); dispatch(adddLike(v.id)); }} className='bg-red-600 text-white py-2 btn px-8 text-xl rounded-xl '>Buy now!</button>
